...
Project.prototype.migrate = function(fn) {
  ...
  self.initFromLock();
  var migrated = {}, nonMigrated = {}, smartJson = this.readSmartJson();
  ...
  _.each(self.dependencies.basePackages, function(pkg) {
    ...
    Atmosphere.package(name, function(pkg) {
      var bail = function() {
        ...
        console.log('');
        var message = 
        console.log('If you want to continue, remove this package from smart.json, run `mrt install`, and try again.'.yellow);
        console.log('After you have successfully migrated, you can add it back but note:'.yellow);
        console.log('  You will NOT receive further updates to this package.'.red);
        console.log('See https://hackpad.com/Migrating-Apps-UfPrM192vSQ for more information.'.yellow);
        process.exit(1);
      }
      
      if (! pkg)
        bail("Error: no package named ", name, " found on Atmosphere");
      ...
      if (! version)
        bail("Error: couldn't find version", searchFor,
          "of package", name, "on Atmosphere");
      
      if (! version.troposphereIdentifier)
        bail("Error: The version", searchFor, 
      ...
      migrated[name] = pkg;
      complete += 1;
      console.log(("1." + complete + ". Updated " + name + " to " + id).green);
      if (complete === _.keys(self.dependencies.basePackages).length)
        done()
    });
  });
